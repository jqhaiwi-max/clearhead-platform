...
import {
  ...
  MessageCircle, CheckSquare, Send, Filter,
} from "lucide-react";
...
function msgConfirm(a: Appointment) {
  return `Hi ${a.patientName}! 🌿\n\nYour appointment with *${a.providerName}* has been *confirmed*.\n\n📅 *Date:* ${a.date}\n🕐 *Time:* ${a.time}\n💬 *Session type:* ${a.type}\n\nWe look forward to your session.\n\n— *Clearhead Team*`;
}

function msgReminder(a: Appointment) {
  return `Hi ${a.patientName}! ⏰\n\nReminder: Your session with *${a.providerName}* is coming up.\n\n📅 *Date:* ${a.date}\n🕐 *Time:* ${a.time}\n\nPlease ensure your device is ready for a ${a.type} session.\n\n— *Clearhead Team*`;
}

function msgCancelled(a: Appointment) {
  return `Hi ${a.patientName},\n\nWe wanted to let you know that your session with *${a.providerName}* on ${a.date} at ${a.time} has been cancelled.\n\nPlease contact us to reschedule at your convenience.\n\n— *Clearhead Team*`;
}

function msgAdminNew(a: Appointment) {
  return `🔔 *New Booking*\n\nPatient: ${a.patientName}\nPhone: ${a.patientPhone || "—"}\nEmail: ${a.patientEmail}\nProvider: ${a.providerName}\nDate: ${a.date} at ${a.time}\nType: ${a.type}\nPayment: ${a.paymentMethod || "—"}\nStatus: ${a.status}`;
}

function WABtn({ href, title, children }: { href: string; title: string; children: React.ReactNode }) {
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" title={title}
      className="inline-flex items-center justify-center p-1.5 rounded-lg bg-[#25D366]/10 text-[#128C7E] hover:bg-[#25D366]/25 transition-colors">
      {children}
    </a>
  );
}
...
function Appointments() {
  ...
  const filtered = appointments.filter(a => {
    ...
    if (search) {
      ...
      if (!a.patientName.toLowerCase().includes(q) && !a.providerName.toLowerCase().includes(q) &&
          !(a.patientPhone ?? "").includes(q)) return false;
    }
    return true;
  ...
  const selectedRows = filtered.filter(a => selected.has(a.id));
  ...
  /* Bulk WhatsApp — send admin a summary of selected rows */
  const bulkWA = () => {
    const lines = selectedRows.map(a => `• ${a.patientName} → ${a.providerName}  ${a.date} ${a.time}  [${a.status}]`).join("\n");
    window.open(`https://wa.me/${ADMIN_WA}?text=${encodeURIComponent(`📋 *Selected Appointments (${selectedRows.length})*\n\n${lines}`)}`, "_blank");
  };
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <div className="flex items-center gap-3 mt-0.5 flex-wrap">
            ...
            {pendingCount > 0 && (
              <span className="flex items-center gap-1.5 text-xs font-semibold text-amber-700 bg-amber-50 border border-amber-200 px-2.5 py-0.5 rounded-full animate-pulse">
                <Clock className="w-3 h-3" /> {pendingCount} awaiting approval
              </span>
            )}
          </div>
        </div>
        <a href={`https://wa.me/${ADMIN_WA}`} target="_blank" rel="noopener noreferrer"
          className="flex items-center gap-2 px-3.5 py-2 bg-[#25D366]/10 text-[#128C7E] border border-[#25D366]/30 rounded-xl text-xs font-semibold hover:bg-[#25D366]/20 transition-colors">
          <MessageCircle className="w-4 h-4" /> Open Admin WhatsApp
        </a>
      </div>

      {/* Status tabs */}
      <div className="flex rounded-xl border border-gray-200 overflow-hidden bg-white text-sm w-fit">
        {TABS.map(t => (
          <button key={t.key} onClick={() => changeFilter(t.key)}
            className={`px-3.5 py-2.5 capitalize transition-colors whitespace-nowrap font-medium ${
              filter === t.key ? "bg-emerald-600 text-white" : "text-gray-600 hover:bg-gray-50"
            } ${t.key === "pending" && pendingCount > 0 && filter !== "pending" ? "text-amber-600 font-semibold" : ""}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="relative flex-1 min-w-52">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
          <input value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Search patient, provider, phone…"
        ...
        <input type="date" value={dateFilter} onChange={e => setDateFilter(e.target.value)}
          className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 bg-white text-gray-700 cursor-pointer" />
        {dateFilter && (
      ...
      {/* Bulk action bar */}
      {selected.size > 0 && (
        <div className="flex items-center gap-2 px-4 py-3 bg-emerald-50 border border-emerald-200 rounded-xl flex-wrap">
          <span className="text-sm font-bold text-emerald-800 mr-1">{selected.size} selected</span>
          <button onClick={() => bulkUpdate.mutate({ ids: selectedIds, status: "confirmed" })}
          ...
          <button onClick={() => bulkUpdate.mutate({ ids: selectedIds, status: "completed" })}
            disabled={bulkUpdate.isPending}
            ...
            <CheckSquare className="w-3.5 h-3.5" /> Complete All
          </button>
          <button onClick={() => bulkUpdate.mutate({ ids: selectedIds, status: "cancelled" })}
            disabled={bulkUpdate.isPending}
          ...
          <button onClick={() => bulkUpdate.mutate({ ids: selectedIds, status: "pending" })}
            disabled={bulkUpdate.isPending}
            ...
            <Clock className="w-3.5 h-3.5" /> Reset Pending
          </button>
          <button onClick={bulkWA}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#25D366]/15 text-[#128C7E] border border-[#25D366]/30 text-xs font-semibold hover:bg-[#25D366]/25 transition-colors">
            <MessageCircle className="w-3.5 h-3.5" /> WhatsApp Summary
          </button>
          <button onClick={() => setSelected(new Set())} className="ml-auto p-1.5 rounded-lg hover:bg-emerald-100 text-emerald-700 transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
                ...
                return (
                  ...
                  <tr
                    ...
                    <td className="px-3 py-3 min-w-36">
                      ...
                      <p className="text-xs text-gray-500 truncate max-w-32">{a.patientEmail}</p>
                      {a.patientPhone && (
                        <a href={`https://wa.me/${a.patientPhone.replace(/\D/g,"")}`}
                    ...
                    {/* Provider */}
                    <td className="px-3 py-3 font-medium text-gray-800 whitespace-nowrap">{a.providerName}</td>

                    {/* Date + time */}
                    <td className="px-3 py-3 whitespace-nowrap">
                      <p className="font-medium text-gray-900">{a.date}</p>
                      <p className="text-xs text-gray-500 flex items-center gap-1"><Clock className="w-3 h-3" />{a.time}</p>
                    </td>

                    {/* Type */}
                    <td className="px-3 py-3 text-gray-600 capitalize text-xs whitespace-nowrap">{a.type}</td>

                    {/* Payment */}
                    <td className="px-3 py-3 text-xs text-gray-500 whitespace-nowrap">
                      {a.paymentMethod ? METHOD_LABELS[a.paymentMethod] ?? a.paymentMethod : "—"}
                    </td>
                    ...
                    <td className="px-3 py-3">
                      <div className="flex items-center gap-1 flex-wrap">
                        ...
                        {isConfirmed && a.patientPhone && (
                          <WABtn href={waLink(a.patientPhone, msgConfirm(a))} title="Send confirmation via WhatsApp">
                            <MessageCircle className="w-3.5 h-3.5" />
                          </WABtn>
                        )}
                        {/* WhatsApp — reminder to patient */}
                        {isActive && a.patientPhone && (
                          <WABtn href={waLink(a.patientPhone, msgReminder(a))} title="Send reminder via WhatsApp">
                            <Send className="w-3.5 h-3.5" />
                          </WABtn>
                        )}
                        {/* WhatsApp — notify admin of new pending */}
                        {isPending && (
                          <WABtn href={`https://wa.me/${ADMIN_WA}?text=${encodeURIComponent(msgAdminNew(a))}`} title="Send alert to admin WhatsApp">
                            <Phone className="w-3.5 h-3.5" />
                          </WABtn>
                        )}
                        {/* WhatsApp — cancellation notice */}
                        {a.status === "cancelled" && a.patientPhone && (
                          <WABtn href={waLink(a.patientPhone, msgCancelled(a))} title="Send cancellation notice">
                            <MessageCircle className="w-3.5 h-3.5" />
                          </WABtn>
                        )}
                      </div>
